...
//                info.showAndWait();
        ...
        // equation subMenu
        MenuItem equationSearchRoots = new MenuItem("Search minimum");
        MenuItem equationClearRoots = new MenuItem("Clear");
            ...
            if (solveEquationController.isValuesPresent()) {
                ...
                if (roots.size() == 0) {
                    ...
                    for (Point2D arg : roots) {
                        rootsString += String.format("x : %1$.5f\t\tmin : %2$.5f\n", arg.getX(), arg.getY());
                    }//просто вівести

                    alert.setContentText("Roots : \n\n" + rootsString);
                    Optional<ButtonType> alertResult = alert.showAndWait();
                    if (alertResult.isPresent() && alertResult.get() == saveReport) {
                        canvasGraphic.reSetCoordinateSystem();//костыль !!!
                        saveReport(roots);
                        String url = PROJECT_PATH + "/src/reports/report_" + (0 + 1) + ".html";
        ...
        minimum.getItems().addAll(equationSearchRoots, equationClearRoots);
        ...
        menuBar.getMenus().addAll(file, graphic, minimum, help);
    }
...
//        int countOfReports = new File(PROJECT_PATH + "/src/reports/countOfReports.txt").exists() ? Integer.parseInt(ReadWrite.readMainLanguage(PROJECT_PATH + "/src/reports/countOfReports.txt")) : 0;
        ...
        File file = new File(PROJECT_PATH + "/src/reports/screenshots/screenshot_" + ".png");
//        ReadWrite.writeMainLanguage((countOfReports + 1) + "", PROJECT_PATH + "/src/reports/countOfReports.txt");
        ...
        sb.append("\n<h1>Снимок экрана :</h1>");
        sb.append("<center><img src=\"screenshots/screenshot_" + ".png\" alt=\"picture\"></center>");
        ...
        ReadWrite.writeMainLanguage(new String(sb), PROJECT_PATH + "/src/reports/report_" + (0 + 1) + ".html");
